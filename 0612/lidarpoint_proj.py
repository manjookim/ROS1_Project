...
# !!! 중요 수정 !!!
# t_L2C의 Z 값이 -14.79이면 대부분의 LiDAR 포인트가 카메라 뒤에 있게 됩니다.
# 따라서 point_3d_camera[2]가 음수가 되어 cv2.projectPoints에서 필터링됩니다.
# 임시로 t_L2C의 Z 값을 양수로 조정하여 점이 보이도록 합니다.
# 이 값은 실제 캘리브레이션 결과와 환경에 따라 정확히 맞춰져야 합니다.
t_L2C = np.array([[-3.63888619],
                  [-0.78565351],
                  [1.0]], dtype=np.float32) # Z 값을 1.0으로 임시 변경
...
# 원본 이미지 로드
image_path = '/home/user/Downloads/school_classes/scripts/pair_output/images/frame_0052.png' # 실제 이미지 경로로 변경하세요!
...
# 3. LiDAR 포인트 투영
projected_points = []
# colors = [] # 시각화를 위한 색상 (깊이에 따라) - 이제 사용하지 않고 고정 색상으로 그림
...
# 각 LiDAR 포인트를 처리
for i, point_3d_lidar in enumerate(lidar_points): # i를 추가하여 디버깅 정보에 활용
    # 1. LiDAR 좌표계 -> 카메라 좌표계 (이 단계는 직접 투영에는 필수적이지 않지만, Z값 확인에 유용)
    point_3d_camera = np.dot(R_L2C, point_3d_lidar.reshape(3, 1)) + t_L2C
    
    # 카메라 앞에 있는지 확인 (Z > 0)
    # Z 값이 0에 너무 가까우면 투영 시 문제가 발생할 수 있으므로 약간의 여유를 줍니다.
    if point_3d_camera[2] > 0.01: # Z > 0 대신 Z > 0.01로 더 안전하게 검사
        ...
        # 픽셀 좌표 추출
        u, v = int(round(img_points[0][0][0])), int(round(img_points[0][0][1])) # 반올림 후 정수 변환

        # 이미지 경계 내에 있는지 확인
        if 0 <= u < image.shape[1] and 0 <= v < image.shape[0]:
            projected_points.append((u, v))
            # 깊이(Z 값)에 따른 색상 (선택 사항) - 이제 고정 색상으로 그림
            ...
            # colors.append(color)
        # else:
        #    print(f"디버그: {i}번 LiDAR 포인트 ({point_3d_lidar})는 이미지 경계 밖으로 투영됨: ({u}, {v})")
    # else:
    #    print(f"디버그: {i}번 LiDAR 포인트 ({point_3d_lidar})는 카메라 뒤에 있거나 너무 가까움 (Z={point_3d_camera[2][0]:.2f})")

# 이미지에 투영된 포인트 그리기
output_image = image.copy()

# 디버깅 정보 출력
print(f"\n투영된 점의 개수: {len(projected_points)}")
if len(projected_points) > 0:
    print(f"첫 5개 투영된 점의 좌표 (u,v):")
    for j in range(min(5, len(projected_points))):
        print(f"  점 {j}: {projected_points[j]}")
else:
    print("경고: 투영된 점 배열이 비어있습니다. 이미지에 그려질 점이 없습니다.")

# 투영된 점이 있을 경우에만 그림
if len(projected_points) > 0:
    for i, (u, v) in enumerate(projected_points):
        # 모든 점을 초록색, 반지름 5로 그립니다. (시각화 확인용)
        cv2.circle(output_image, (u, v), 5, (0, 255, 0), -1) # 반지름 5, 초록색, 채우기
else:
    print("투영된 점이 없으므로 이미지에 그려진 점이 없습니다.")
